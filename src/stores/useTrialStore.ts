    ...
    (set, get) => ({
      ...
      purchaseLifetimeAccess: async (): Promise<{ success: boolean; error?: string }> => {
        try {
          ...
          // REMOVED: Blocking null check - let AppleIAPService handle auto-init
          // The Build 95 auto-initialization in AppleIAPService.purchase() will handle this

          console.log('💳 Processing purchase via', state.paymentService?.provider || 'payment service');

          // If paymentService is somehow still null, create it now
          ...
          // AppleIAPService.purchase() has auto-init built in (Build 95)
          const result = await get().paymentService!.purchase('com.readingdaily.lifetime.access.v2');
      ...
      restorePurchase: async (): Promise<boolean> => {
        try {
          ...
          console.log('🔄 Restoring purchases via', state.paymentService?.provider || 'payment service');
          ...
          const result = await get().paymentService!.restorePurchases();
      ...
      upgradeToBasic: async (subscriptionId: string): Promise<boolean> => {
        try {
          ...
          console.log('💳 Processing subscription upgrade via', state.paymentService?.provider || 'payment service');
          ...
          const result = await get().paymentService!.purchase(subscriptionId);
